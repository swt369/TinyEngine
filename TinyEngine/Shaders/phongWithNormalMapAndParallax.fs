...
vec3 calcDirLight(int index, vec3 normal_tangent, vec3 viewDir_tangent, vec2 uv);
vec3 calcPointLight(int index, vec3 normal_tangent, vec3 viewDir_tangent, vec2 uv);
vec3 calcSpotLight(int index, vec3 normal_tangent, vec3 viewDir_tangent, vec2 uv);
float calcDirectionalShadow(int index);
float calcOmniDirectionalShadow(int index);
vec2 calcParallaxMapping(vec2 uv, vec3 viewDir);
...
void main()
{
	...
	vec2 uv_parallaxed = calcParallaxMapping(uv, viewDir_tangent);
	if (uv_parallaxed.x > 1.0 || uv_parallaxed.y > 1.0 || uv_parallaxed.x < 0.0 || uv_parallaxed.y < 0.0)
		discard;
	
	vec3 resultColor = vec3(0.0);
	for (int i = 0; i < directionalLightCount; i++)
		resultColor += calcDirLight(i, normal_tangent, viewDir_tangent, uv_parallaxed);
		
	for (int i = 0; i < pointLightCount; i++)
		resultColor += calcPointLight(i, normal_tangent, viewDir_tangent, uv_parallaxed);
		
	for (int i = 0; i < spotLightCount; i++)
		resultColor += calcSpotLight(i, normal_tangent, viewDir_tangent, uv_parallaxed);
